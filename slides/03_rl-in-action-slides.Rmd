...
- Most of the time we *exploit* our knowledge with $pr = 1-\epsilon$, i.e. choose the action which gives us the highest estimated reward and update the value of a state using $$\begin{align}V(S_t) &= V(S_t) + \alpha(V(S_{t+1})-V(S_t)) \\&= (1-\alpha)V(S_t) + \alpha V(S_{t+1})\end{align}$$ where $\alpha$ is the *step-size* parameter. 
