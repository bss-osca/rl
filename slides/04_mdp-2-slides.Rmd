                         ...
                         radx = 0.03, rady=0.05, cex=1, marX=0.035, marY=0.15, ...)
{
   ...
   if (!is.null(actions)) {
      for (i in seq_along(actions)) {
         ...
         parseLabel <- if_else(is.null(actions[[i]]$parseLabel), T, actions[[i]]$parseLabel)
...
* The objective function of an MDP which is to find an optimal policy $\pi_*$ with state-value function:

$$v_*(s) = \max_\pi v_\pi(s).$$
* A policy $\pi'$ is better than policy $\pi$ if its expected return is higher for all states. 
...
* May though be expensive to solve the equations if the number of states is huge. 
