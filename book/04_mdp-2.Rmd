...
<!-- Q1 -->
```{r, solution=TRUE, text="$$\\mathcal{S} = \\{ (x,y) | 0 \\leq x \\leq 20, 0 \\leq y \\leq 20 \\}$$"}
```

   1) Define the state space (with states $(x,y)$) equal the number of cars at each location at the end of the day.
   
<!-- Q2 -->
```{r, solution=TRUE, text="$$\\mathcal{A}(s) = \\{ a | -\\min(5,y,20-x) \\leq a \\leq min(5,x,20-y) \\}$$"}
```
...
<!-- Q3 -->
```{r, solution=TRUE, text="The reward equals the reward of rentals minus the cost of movements. Note we have $\\tilde{x} = x - a$ and $\\tilde{y} = x + a$ after movement. Hence $$r(s,a) = \\mathbb{E}[10(\\min(D_1, \\tilde{x}) + \\min(D_2, \\tilde{y}) )-2\\mid a \\mid]$$ where $$\\mathbb{E}[\\min(D, z)] = \\sum_{i=0}^z ip(D = i) + (1-p(D\\leq z))z.$$"}
```
...
<!-- Q4 -->
```{r, solution=TRUE, text="Only difference is that cars moved to Location 2 is $a$ (and not $-a$): $$Y = \\min(20, y + a - \\min(D_2, y+a) + H_2)).$$"}
```
