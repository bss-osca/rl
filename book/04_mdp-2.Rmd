...
<!-- Q1 -->
```{r, solution=TRUE, text="$$\\mathcal{S} = \\{ (x,y) | 0 \\leq x, y \\leq 20 \\}$$"}
```

   1) Define the state space (with states $(x,y)$) equal the number of cars at each location at the end of the day.
   
<!-- Q2 -->
```{r, solution=TRUE, text="$$\\mathcal{A}(s) = \\{ a | -\\min(5,y) \\leq a \\leq min(5,x) \\}$$"}
```
...
<!-- Q3 -->
```{r, solution=TRUE, text="The reward equals the reward of rentals minus the cost of movements: $$r(s,a) = \\mathbb{E}[10(D_1+D_2)-2|a|] = 10(\\lambda_{D_1}+\\lambda_{D_2})-2|a|$$"}
```
...
<!-- Q4 -->
```{r, solution=TRUE, text="Only difference is that cars moved to Location 2 is $a$ (and not $-a$): $$Y = \\min(20, \\max(0, y' + a - D_2) + H_2)))$$"}
```
