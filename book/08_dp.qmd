...
Let us consider Exercise @exr-mdp-1-storage where a factory has a storage tank with a capacity of 4 $\mathrm{m}^{3}$ for temporarily storing waste produced by the factory. Each week the factory produces $0,1$, 2 or 3 $\mathrm{m}^{3}$ waste with respective probabilities 
...
### Exercise - Gambler's problem {#exr-dp-gambler}

Consider the gambler's problem in @exr-mdp-1-gambler.

<!-- Q1 -->
```{r gambler-build, solution = TRUE, include=FALSE, eval=FALSE}
...
```{r, solution=TRUE, text = "We make a function that return the mdp object"}
...
<!-- Q2 -->
```{r gambler-solve, solution=TRUE, text="There is a lot of different optimal policies. In general since $p_h$ is below 0.5 we want to find a policy with as few flips as possible."}
...
```{r gambler-mdp2, solution=TRUE, include=FALSE, eval=FALSE}
...
### Exercise - Maintenance problem {#exr-dp-maintain}
...
### Exercise - Car rental {#exr-dp-rental}
