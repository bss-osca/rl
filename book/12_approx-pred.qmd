...
labelled 1 through 1000. Two terminal states lie just outside this range:
...
From any non-terminal state $s$, the agent flips a fair coin to choose
...
linear in parameters $\textbf w$ even though it is non-linear in $s$.
...
too high a degree can lead to over-fitting and poor generalization.
...
instabilities. If we normalize so state variables are in the interval
...
parameters* even though it can represent highly non-linear shapes. The
...
$\mathbf c=(c_1,\dots,c_k)$ with non-negative integers. Combinations can
be limited by considering $c$ where $c_k\in 0,\ldots, n$, i.e. then number
of features are $m = (n+1)^k$. Given the $m$ frequency vectors, the feature vector for the $i$'th $c^i$ now becomes

$$
x_{\mathbf c^i}(\mathbf z)=\cos\big(\pi  z^\top c^i\big).
$$
...
tilings; each tiling itself does not overlap (it is a partition of the
...
For instance, for tile coding with $n$ tilings we have that
